...
②、再点击BridgeSvr.exe运行，然后在电脑右下角找到图标鼠标右键->显示桥界面，切换到“关于”页面点击一键拷贝等待结束；

③、再去各个MT4/5导航窗口点击脚本或服务，鼠标右键刷新；

④、如果是MT5，首次点鼠标右键"添加服务"，以后每次点鼠标右键"启动"服务。如果是MT4把脚本添加到价格图表。
...
③、一拖N也可，N拖M也可(即多个账户同时做信号源)，互不干扰，角色你自己任意分配，可以同向跟单、反向跟单、按比例倍数跟单，任意设置，建多层级联跟单链。
...
1、API跟单模式:最大弊端是所有账户需先登记采集经纪商名/服务器/账号/交易密码，控制权交出去了，极不安全，如果有亲朋好友要跟你的单，他们必须把服务器名/账号/交易密码先交给你你再添加到你API跟单软件上，显然他们不愿意给的。而且跟单账号数量一多就导致电脑非常卡顿。

2、本地EA模式:使用用户登录好的MT4/5下单，不采集隐私，但所有账号须在同一电脑上运行不能跨主机，参差不齐，品种后缀还要人工设置映射，要挂载到价格图表上，且最大弊端就是EA是每个tick都运行一次，非常卡顿，非常占CPU，一台电脑运行两三个EA，电脑就卡顿得不行了。


3、[桥+服务]模式:即本跟单软件独创做法，同样使用用户已登录好的MT4/5下单，用桥将所有账号互连互通，实现跨网络跟单，软件自动适配品种后缀无需设置映射。如果有亲朋好友想要跟你的单，你仅需把slave那两个文件发给他们自行放到电脑MT4/5对应目录下再启动服务，并按F3把BridgeSvrIP填你的桥在的云主机IP，就可以跟你的单了。如果你的亲朋好友也想当信号源，就把master那两个文件发给他们放到对应目录启动服务并按F3设置好BridgeSvrIP就可以了。由于是本跟单软件是服务类型(不是EA)，且是全异步模型，所以完全不占用CPU,完全不占内存。而且本跟单软件解决了其他跟单软件存在严重漏单的问题，特别是信号源台子非常快，但跟随端台子非常慢的情形。比如：信号源订单开单成功，两百毫秒之后就又发送了修改了止盈止损指令，而跟随端开单指令丢出去卡了10秒了开仓订单都还没成功返回，此时又已提前收到了修改了止盈止损指令。再比如：信号源订单开单后5秒就平仓了，而跟随端开仓指令丢出去10秒还没返回开仓成功的订单，就提前收到了平仓信号……，这些情况其它跟单软件必会漏单。这种只能用异步模型，就是先要把信号源来的订单信号都保存下来，在后面跟单端开仓订单成功后做跟踪处理。本跟单软件一开始设计就采用这种异步做法，最大幅度减少了漏单的情况。
...
④、System32：此目录3个DLL是微软运行时库。强烈建议直接安装包里自带的微软运行时库合集：MSVBCRT_AllInOne，可解决遇到加载服务时提示DLL无法加载的问题。
